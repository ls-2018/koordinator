...
type nodeMetricInformer struct {
	...
	predictorFactory over_prediction.PredictorFactory
...
func (r *nodeMetricInformer) queryNodeMetric(start time.Time, end time.Time, aggregateType over_metriccache.AggregationType,
...
func (r *nodeMetricInformer) collectNodeMetric(queryparam over_metriccache.QueryParam) (corev1.ResourceList, time.Duration, error) {
	...
	cpuAggregateResult, err := doQuery(querier, over_metriccache.NodeCPUUsageMetric, nil)
	if err != nil {
		return rl, 0, err
	}
	...
	var memAggregateResult over_metriccache.AggregateResult
	...
	if nodeMemoryCollectPolicy == slov1alpha1.UsageWithoutPageCache {
		...
		memAggregateResult, err = doQuery(querier, over_metriccache.NodeMemoryUsageMetric, nil)
		if err != nil {
			return rl, 0, err
		}
		...
		memAggregateResult, err = doQuery(querier, over_metriccache.NodeMemoryWithHotPageUsageMetric, nil)
		if err != nil {
			return rl, 0, err
		}
	} else if nodeMemoryCollectPolicy == slov1alpha1.UsageWithPageCache {
		// report usageMemoryWithPageCache
		memAggregateResult, err = doQuery(querier, over_metriccache.NodeMemoryUsageWithPageCacheMetric, nil)
		if err != nil {
			return rl, 0, err
		}
	} else {
...
func (r *nodeMetricInformer) collectNodeGPUMetric(queryparam over_metriccache.QueryParam, gpus koordletutil.GPUDevices) ([]schedulingv1alpha1.DeviceInfo, error) {
	...
	for _, gpu := range gpus {
		gpuCoreUsageAggregateResult, err := doQuery(querier, over_metriccache.NodeGPUCoreUsageMetric, over_metriccache.MetricPropertiesFunc.GPU(fmt.Sprintf("%d", gpu.Minor), gpu.UUID))
		if err != nil {
			return result, err
		}
		...
		gpuMemUsedAggregateResult, err := doQuery(querier, over_metriccache.NodeGPUMemUsageMetric, over_metriccache.MetricPropertiesFunc.GPU(fmt.Sprintf("%d", gpu.Minor), gpu.UUID))
		if err != nil {
			return result, err
		}
	...
	for _, d := range aggregatePolicy.Durations {
		...
		aggregateUsage := slov1alpha1.AggregatedUsage{
			Usage: map[apiext.AggregationType]slov1alpha1.ResourceMap{
				apiext.P50: r.queryNodeMetric(start, endTime, over_metriccache.AggregationTypeP50, true),
				apiext.P90: r.queryNodeMetric(start, endTime, over_metriccache.AggregationTypeP90, true),
				apiext.P95: r.queryNodeMetric(start, endTime, over_metriccache.AggregationTypeP95, true),
				apiext.P99: r.queryNodeMetric(start, endTime, over_metriccache.AggregationTypeP99, true),
			},
			Duration: d,
		}
...
func (r *nodeMetricInformer) collectSystemMetric(queryparam over_metriccache.QueryParam) (corev1.ResourceList, time.Duration, error) {
	rl := corev1.ResourceList{}
	querier, err := r.metricCache.Querier(*queryparam.Start, *queryparam.End)
	if err != nil {
	...
	cpuAggregateResult, err := doQuery(querier, over_metriccache.SystemCPUUsageMetric, nil)
	if err != nil {
		return rl, 0, err
	}
	cpuUsed, err := cpuAggregateResult.Value(queryparam.Aggregate)
	...
	memAggregateResult, err := doQuery(querier, over_metriccache.SystemMemoryUsageMetric, nil)
	if err != nil {
		return rl, 0, err
	}
...
func (r *nodeMetricInformer) querySystemMetric(start time.Time, end time.Time, aggregateType over_metriccache.AggregationType,
	coldStartFilter bool) slov1alpha1.ResourceMap {
	rm := slov1alpha1.ResourceMap{}
...
func (r *nodeMetricInformer) collectSystemAggregateMetric(endTime time.Time, aggregatePolicy *slov1alpha1.AggregatePolicy) []slov1alpha1.AggregatedUsage {
	...
	for _, d := range aggregatePolicy.Durations {
		...
		aggregateUsage := slov1alpha1.AggregatedUsage{
			Usage: map[apiext.AggregationType]slov1alpha1.ResourceMap{
				apiext.P50: r.querySystemMetric(start, endTime, over_metriccache.AggregationTypeP50, true),
				apiext.P90: r.querySystemMetric(start, endTime, over_metriccache.AggregationTypeP90, true),
				apiext.P95: r.querySystemMetric(start, endTime, over_metriccache.AggregationTypeP95, true),
				apiext.P99: r.querySystemMetric(start, endTime, over_metriccache.AggregationTypeP99, true),
			},
			Duration: d,
		}
		aggregateUsages = append(aggregateUsages, aggregateUsage)
...
func (r *nodeMetricInformer) collectPodMetric(podMeta *over_statesinformer.PodMeta, queryParam over_metriccache.QueryParam) (*slov1alpha1.PodMetricInfo, error) {
	...
	cpuAggregateResult, err := doQuery(querier, over_metriccache.PodCPUUsageMetric, over_metriccache.MetricPropertiesFunc.Pod(podUID))
	if err != nil {
		return nil, err
	}
	cpuUsed, err := cpuAggregateResult.Value(queryParam.Aggregate)
	...
	if nodeMemoryCollectPolicy == slov1alpha1.UsageWithHotPageCache && system.GetIsStartColdMemory() {
		memAggregateResult, err = doQuery(querier, over_metriccache.PodMemoryWithHotPageUsageMetric, over_metriccache.MetricPropertiesFunc.Pod(podUID))
		if err != nil {
			return nil, err
		}
	} else if nodeMemoryCollectPolicy == slov1alpha1.UsageWithPageCache {
		memAggregateResult, err = doQuery(querier, over_metriccache.PodMemoryUsageWithPageCacheMetric, over_metriccache.MetricPropertiesFunc.Pod(podUID))
		if err != nil {
			return nil, err
		}
	} else { // slov1alpha1.UsageWithoutPageCache
		memAggregateResult, err = doQuery(querier, over_metriccache.PodMemUsageMetric, over_metriccache.MetricPropertiesFunc.Pod(podUID))
		if err != nil {
			return nil, err
		}
	}
...
func (r *nodeMetricInformer) collectHostAppMetric(hostApp *slov1alpha1.HostApplicationSpec, queryParam over_metriccache.QueryParam) (*slov1alpha1.HostApplicationMetricInfo, error) {
	if hostApp == nil {
	...
	cpuAggregateResult, err := doQuery(querier, over_metriccache.HostAppCPUUsageMetric, over_metriccache.MetricPropertiesFunc.HostApplication(hostApp.Name))
	if err != nil {
		return nil, err
	}
	cpuUsed, err := cpuAggregateResult.Value(queryParam.Aggregate)
	...
	metricResource := over_metriccache.HostAppMemoryUsageMetric
	nodeMemoryCollectPolicy := *r.getNodeMetricSpec().CollectPolicy.NodeMemoryCollectPolicy
	if nodeMemoryCollectPolicy == slov1alpha1.UsageWithHotPageCache && system.GetIsStartColdMemory() {
		metricResource = over_metriccache.HostAppMemoryWithHotPageUsageMetric
	} else if nodeMemoryCollectPolicy == slov1alpha1.UsageWithPageCache {
		metricResource = over_metriccache.HostAppMemoryUsageWithPageCacheMetric
	}

	metricProperties := over_metriccache.MetricPropertiesFunc.HostApplication(hostApp.Name)
...
func (r *nodeMetricInformer) collectPodGPUMetric(queryparam over_metriccache.QueryParam, uid string, gpus koordletutil.GPUDevices) ([]schedulingv1alpha1.DeviceInfo, error) {
	result := make([]schedulingv1alpha1.DeviceInfo, 0)
	querier, err := r.metricCache.Querier(*queryparam.Start, *queryparam.End)
	if err != nil {
	...
	for _, gpu := range gpus {
		properties := over_metriccache.MetricPropertiesFunc.PodGPU(uid, fmt.Sprintf("%d", gpu.Minor), gpu.UUID)
		gpuCoreUsageAggregateResult, err := doQuery(querier, over_metriccache.PodGPUCoreUsageMetric, properties)
		if err != nil {
			return result, err
		}
		if gpuCoreUsageAggregateResult.Count() == 0 {
		...
		gpuMemUsedAggregateResult, err := doQuery(querier, over_metriccache.PodGPUMemUsageMetric, properties)
		if err != nil {
			return result, err
		}
		if gpuMemUsedAggregateResult.Count() == 0 {
...
func (r *nodeMetricInformer) fillGPUMetrics(queryparam over_metriccache.QueryParam, info *slov1alpha1.PodMetricInfo, uid string, gpus koordletutil.GPUDevices) {
...
func doQuery(querier over_metriccache.Querier, resource over_metriccache.MetricResource, properties map[over_metriccache.MetricProperty]string) (over_metriccache.AggregateResult, error) {
...
func (r *nodeMetricInformer) collectMetric() (*slov1alpha1.NodeMetricInfo, []*slov1alpha1.PodMetricInfo,
	...
	nodeMetricInfo := &slov1alpha1.NodeMetricInfo{
		NodeUsage:              r.queryNodeMetric(startTime, endTime, over_metriccache.AggregationTypeAVG, false),
		AggregatedNodeUsages:   r.collectNodeAggregateMetric(endTime, spec.CollectPolicy.NodeAggregatePolicy),
		SystemUsage:            r.querySystemMetric(startTime, endTime, over_metriccache.AggregationTypeAVG, false),
	...
	prodPredictor := r.predictorFactory.New(over_prediction.ProdReclaimablePredictor) // 可回收生产预测器。
	...
	if p, err := prodPredictor.GetResult(); err != nil {
		...
		over_metrics.RecordNodeResourcePriorityReclaimable(string(corev1.ResourceCPU), over_metrics.UnitCore, string(apiext.PriorityProd), 0)
		over_metrics.RecordNodeResourcePriorityReclaimable(string(corev1.ResourceMemory), over_metrics.UnitByte, string(apiext.PriorityProd), 0)
	} else {
		prodReclaimable.Resource = slov1alpha1.ResourceMap{ResourceList: p}
		over_metrics.RecordNodeResourcePriorityReclaimable(string(corev1.ResourceCPU), over_metrics.UnitCore, string(apiext.PriorityProd), float64(p.Cpu().MilliValue())/1000)
		over_metrics.RecordNodeResourcePriorityReclaimable(string(corev1.ResourceMemory), over_metrics.UnitByte, string(apiext.PriorityProd), float64(p.Memory().Value()))
	}
