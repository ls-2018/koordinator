...
import (
	...
	"github.com/koordinator-sh/koordinator/pkg/koordlet/over_pleg"
...
type podsInformer struct {
	...
	podMap         map[string]*over_statesinformer.PodMeta
...
func (s *podsInformer) Setup(ctx *PluginOption, states *PluginState) {
	p, err := over_pleg.NewPLEG(system.Conf.CgroupRootDir)
	if err != nil {
...
func (s *podsInformer) Start(stopCh <-chan struct{}) {
	...
	s.kubelet = stub
	hdlID := s.pleg.AddHandler(over_pleg.PodLifeCycleHandlerFuncs{
...
func (s *podsInformer) GetAllPods() []*over_statesinformer.PodMeta {
	s.podRWMutex.RLock()
	defer s.podRWMutex.RUnlock()
	pods := make([]*over_statesinformer.PodMeta, 0, len(s.podMap))
...
func (s *podsInformer) syncPods() error {
	...
	newPodMap := make(map[string]*over_statesinformer.PodMeta, len(podList.Items))
	// reset pod container metrics
	resetPodMetrics()
	for i := range podList.Items {
		pod := &podList.Items[i]
		podMeta := &over_statesinformer.PodMeta{
	...
	s.callbackRunner.SendCallback(over_statesinformer.RegisterTypeAllPods)
	return nil
}
...
func resetPodMetrics() {
	over_metrics.ResetContainerResourceRequests()
	over_metrics.ResetContainerResourceLimits()
}

func recordPodResourceMetrics(podMeta *over_statesinformer.PodMeta) {
...
func recordContainerResourceMetrics(container *corev1.Container, containerStatus *corev1.ContainerStatus, pod *corev1.Pod) {
	...
	if q, ok := container.Resources.Requests[apiext.BatchCPU]; ok {
		over_metrics.RecordContainerResourceRequests(string(apiext.BatchCPU), over_metrics.UnitInteger, containerStatus, pod, float64(util.QuantityPtr(q).Value()))
	}
	if q, ok := container.Resources.Requests[apiext.BatchMemory]; ok {
		over_metrics.RecordContainerResourceRequests(string(apiext.BatchMemory), over_metrics.UnitInteger, containerStatus, pod, float64(util.QuantityPtr(q).Value()))
	}
	if q, ok := container.Resources.Limits[apiext.BatchCPU]; ok {
		over_metrics.RecordContainerResourceLimits(string(apiext.BatchCPU), over_metrics.UnitByte, containerStatus, pod, float64(util.QuantityPtr(q).Value()))
	}
	if q, ok := container.Resources.Limits[apiext.BatchMemory]; ok {
		over_metrics.RecordContainerResourceLimits(string(apiext.BatchMemory), over_metrics.UnitByte, containerStatus, pod, float64(util.QuantityPtr(q).Value()))
	}
	// record pod requests/limits of MidCPU & MidMemory
	if q, ok := container.Resources.Requests[apiext.MidCPU]; ok {
		over_metrics.RecordContainerResourceRequests(string(apiext.MidCPU), over_metrics.UnitInteger, containerStatus, pod, float64(util.QuantityPtr(q).Value()))
	}
	if q, ok := container.Resources.Requests[apiext.MidMemory]; ok {
		over_metrics.RecordContainerResourceRequests(string(apiext.MidMemory), over_metrics.UnitInteger, containerStatus, pod, float64(util.QuantityPtr(q).Value()))
	}
	if q, ok := container.Resources.Limits[apiext.MidCPU]; ok {
		over_metrics.RecordContainerResourceLimits(string(apiext.MidCPU), over_metrics.UnitByte, containerStatus, pod, float64(util.QuantityPtr(q).Value()))
	}
	if q, ok := container.Resources.Limits[apiext.MidMemory]; ok {
		over_metrics.RecordContainerResourceLimits(string(apiext.MidMemory), over_metrics.UnitByte, containerStatus, pod, float64(util.QuantityPtr(q).Value()))
	}
}
