...
type updateCallback struct {
	name        string
	description string
	fn          over_statesinformer.UpdateCbFn
}

type UpdateCbCtx struct{}

type callbackRunner struct {
	callbackChans        map[over_statesinformer.RegisterType]chan UpdateCbCtx
	stateUpdateCallbacks map[over_statesinformer.RegisterType][]updateCallback
	statesInformer       StatesInformer
}

func NewCallbackRunner() *callbackRunner {
	c := &callbackRunner{}
	c.callbackChans = map[over_statesinformer.RegisterType]chan UpdateCbCtx{
		over_statesinformer.RegisterTypeNodeSLOSpec:  make(chan UpdateCbCtx, 1),
		over_statesinformer.RegisterTypeAllPods:      make(chan UpdateCbCtx, 1),
		over_statesinformer.RegisterTypeNodeTopology: make(chan UpdateCbCtx, 1),
		over_statesinformer.RegisterTypeNodeMetadata: make(chan UpdateCbCtx, 1),
	}
	c.stateUpdateCallbacks = map[over_statesinformer.RegisterType][]updateCallback{
		over_statesinformer.RegisterTypeNodeSLOSpec:  {},
		over_statesinformer.RegisterTypeAllPods:      {},
		over_statesinformer.RegisterTypeNodeTopology: {},
		over_statesinformer.RegisterTypeNodeMetadata: {},
	}
	return c
}
...
func (s *callbackRunner) RegisterCallbacks(rType over_statesinformer.RegisterType, name, description string, callbackFn over_statesinformer.UpdateCbFn) {
...
func (s *callbackRunner) SendCallback(objType over_statesinformer.RegisterType) {
...
func (s *callbackRunner) runCallbacks(objType over_statesinformer.RegisterType, obj interface{}) {
	callbacks, exist := s.stateUpdateCallbacks[objType]
	if !exist {
		klog.Errorf("states informer callbacks type %v not exist", objType.String())
		return
	}
	callbackTarget := &over_statesinformer.CallbackTarget{
...
func (s *callbackRunner) getObjByType(objType over_statesinformer.RegisterType, cbCtx UpdateCbCtx) interface{} {
	switch objType {
	case over_statesinformer.RegisterTypeNodeSLOSpec:
		nodeSLO := s.statesInformer.GetNodeSLO()
		if nodeSLO != nil {
			return &nodeSLO.Spec
		}
		return nil
	case over_statesinformer.RegisterTypeAllPods:
		return &struct{}{}
	case over_statesinformer.RegisterTypeNodeTopology:
		return s.statesInformer.GetNodeTopo()
	case over_statesinformer.RegisterTypeNodeMetadata:
