...
func (s *nodeTopoInformer) calGuaranteedCpu(usedCPUs map[int32]*extension.CPUInfo, stateJSON string) ([]extension.PodCPUAlloc, error) {
	...
	pods := make(map[types.UID]*over_statesinformer.PodMeta)
...
func (s *nodeTopoInformer) calCPUTopology() (*over_metriccache.NodeCPUInfo, *extension.CPUTopology, map[int32]*extension.CPUInfo, error) {
	nodeCPUInfoRaw, exist := s.metricCache.Get(over_metriccache.NodeCPUInfoKey)
	if !exist {
...
func (s *nodeTopoInformer) calTopologyZoneList(nodeCPUInfo *over_metriccache.NodeCPUInfo) (v1alpha1.ZoneList, error) {
	nodeNUMAInfoRaw, exist := s.metricCache.Get(over_metriccache.NodeNUMAInfoKey)
	if !exist {
...
func (s *nodeTopoInformer) updateNodeTopo(newTopo *v1alpha1.NodeResourceTopology) {
	...
	s.callbackRunner.SendCallback(over_statesinformer.RegisterTypeNodeTopology)
}
