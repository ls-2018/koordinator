...
type StatesInformer interface {
	...
	GetAllPods() []*over_statesinformer.PodMeta

	GetNodeTopo() *topov1alpha1.NodeResourceTopology

	GetVolumeName(pvcNamespace, pvcName string) string

	RegisterCallbacks(objType over_statesinformer.RegisterType, name, description string, callbackFn over_statesinformer.UpdateCbFn)
}
...
type statesInformer struct {
	...
	config       *Config
	metricsCache over_metriccache.MetricCache
...
func NewStatesInformer(config *Config, kubeClient clientset.Interface, crdClient koordclientset.Interface, topologyClient topologyclientset.Interface,
	metricsCache over_metriccache.MetricCache, nodeName string, schedulingClient schedv1alpha1.SchedulingV1alpha1Interface, predictorFactory over_prediction.PredictorFactory) StatesInformer {
	opt := &PluginOption{
		config:      config,
...
func (s *statesInformer) GetAllPods() []*over_statesinformer.PodMeta {
...
func (s *statesInformer) RegisterCallbacks(rType over_statesinformer.RegisterType, name, description string, callbackFn over_statesinformer.UpdateCbFn) {
