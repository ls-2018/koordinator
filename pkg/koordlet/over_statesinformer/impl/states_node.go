...
func (s *nodeInformer) syncNode(newNode *corev1.Node) {
	...
	if isNodeMetadataUpdated(s.node, newNode) {
		...
		s.callbackRunner.SendCallback(over_statesinformer.RegisterTypeNodeMetadata)
	}
...
func recordNodeResources(node *corev1.Node) {
	...
	over_metrics.RecordNodeResourceAllocatable(string(apiext.BatchCPU), over_metrics.UnitInteger, float64(batchCPU.Value()))
	batchMemory := node.Status.Allocatable.Name(apiext.BatchMemory, resource.BinarySI)
	over_metrics.RecordNodeResourceAllocatable(string(apiext.BatchMemory), over_metrics.UnitByte, float64(batchMemory.Value()))

	// record node allocatable of MidCPU & MidMemory
	midCPU := node.Status.Allocatable.Name(apiext.MidCPU, resource.DecimalSI)
	over_metrics.RecordNodeResourceAllocatable(string(apiext.MidCPU), over_metrics.UnitInteger, float64(midCPU.Value()))
	midMemory := node.Status.Allocatable.Name(apiext.MidMemory, resource.BinarySI)
	over_metrics.RecordNodeResourceAllocatable(string(apiext.MidMemory), over_metrics.UnitByte, float64(midMemory.Value()))
}
