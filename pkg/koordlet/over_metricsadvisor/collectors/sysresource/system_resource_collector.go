...
func (s *systemResourceCollector) Setup(c *over_framework.Context) {
...
func (s *systemResourceCollector) collectSysResUsed() {
	...
	systemCPUMetric, err := over_metriccache.SystemCPUUsageMetric.GenerateSample(nil, collectTime, systemCPUUsage)
	if err != nil {
		klog.Warningf("generate system cpu metric failed, err %v", err)
		return
	}
	systemMemoryMetric, err := over_metriccache.SystemMemoryUsageMetric.GenerateSample(nil, collectTime, systemMemoryUsage)
	if err != nil {
	...
	if err := appender.Append([]over_metriccache.MetricSample{systemCPUMetric, systemMemoryMetric}); err != nil {
