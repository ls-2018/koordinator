...
func (n *nodeInfoCollector) collectNodeCPUInfo() error {
	...
	if err != nil {
		over_metrics.RecordCollectNodeCPUInfoStatus(err)
		return err
	}

	nodeCPUInfo := &over_metriccache.NodeCPUInfo{
	...
	n.storage.Set(over_metriccache.NodeCPUInfoKey, nodeCPUInfo)
	klog.V(4).Infof("collectNodeCPUInfo finished, processors num %v", len(nodeCPUInfo.ProcessorInfos))
	over_metrics.RecordCollectNodeCPUInfoStatus(nil)
	return nil
}
...
func (n *nodeInfoCollector) collectNodeNUMAInfo() error {
	...
	if err != nil {
		over_metrics.RecordCollectNodeNUMAInfoStatus(err)
		return err
	}
	...
	n.storage.Set(over_metriccache.NodeNUMAInfoKey, nodeNUMAInfo)
	klog.V(4).Infof("collectNodeNUMAInfo finished, NUMA node num %v", len(nodeNUMAInfo.NUMAInfos))
	over_metrics.RecordCollectNodeNUMAInfoStatus(nil)
	return nil
}
