...
type podResourceCollector struct {
	...
	deviceCollectors map[string]over_framework.DeviceCollector
...
func New(opt *over_framework.Options) over_framework.Collector {
	...
	podFilter := over_framework.DefaultPodFilter
	...
	return &podResourceCollector{
		...
		podFilter:            podFilter,
		lastPodCPUStat:       gocache.New(collectInterval*over_framework.ContextExpiredRatio, over_framework.CleanupInterval),
		lastContainerCPUStat: gocache.New(collectInterval*over_framework.ContextExpiredRatio, over_framework.CleanupInterval),
	}
}

var _ over_framework.PodCollector = &podResourceCollector{}

func (p *podResourceCollector) Enabled() bool {
	return true
}

func (p *podResourceCollector) Setup(c *over_framework.Context) {
...
func (p *podResourceCollector) Run(stopCh <-chan struct{}) {
	devicesSynced := func() bool {
		return over_framework.DeviceCollectorsStarted(p.deviceCollectors)
	}
...
func (p *podResourceCollector) FilterPod(meta *over_statesinformer.PodMeta) (bool, string) {
...
func (p *podResourceCollector) collectPodResUsed() {
	...
	allCPUUsageCores := over_metriccache.Point{Timestamp: time.Now(), Value: 0}
	allMemoryUsage := over_metriccache.Point{Timestamp: time.Now(), Value: 0}
	for _, meta := range podMetas {
		...
		p.lastPodCPUStat.Set(uid, over_framework.CPUStat{
		...
		cpuUsageMetric, err := over_metriccache.PodCPUUsageMetric.GenerateSample(
			over_metriccache.MetricPropertiesFunc.Pod(uid), collectTime, cpuUsageValue)
		if err != nil {
		...
		memUsageMetric, err := over_metriccache.PodMemUsageMetric.GenerateSample(
			over_metriccache.MetricPropertiesFunc.Pod(uid), collectTime, float64(memUsageValue))
		if err != nil {
...
func (p *podResourceCollector) collectContainerResUsed(meta *over_statesinformer.PodMeta) []over_metriccache.MetricSample {
	klog.V(6).Infof("start collectContainerResUsed")
	pod := meta.Pod
	count := 0
	containerMetrics := make([]over_metriccache.MetricSample, 0)
	for i := range pod.Status.ContainerStatuses {
		...
		p.lastContainerCPUStat.Set(containerStat.ContainerID, over_framework.CPUStat{
			CPUUsage:  currentCPUUsage,
			Timestamp: collectTime,
		}, gocache.DefaultExpiration)
		...
		cpuUsageMetric, cpuErr := over_metriccache.ContainerCPUUsageMetric.GenerateSample(
			over_metriccache.MetricPropertiesFunc.Container(containerStat.ContainerID), collectTime, cpuUsageValue)

		memUsageValue := memStat.Usage()
		memUsageMetric, memErr := over_metriccache.ContainerMemUsageMetric.GenerateSample(
			over_metriccache.MetricPropertiesFunc.Container(containerStat.ContainerID), collectTime, float64(memUsageValue))
