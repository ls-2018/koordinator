...
type nodeResourceCollector struct {
	...
	lastNodeCPUStat *over_framework.CPUStat
...
func (n *nodeResourceCollector) Setup(c *over_framework.Context) {
	n.deviceCollectors = c.DeviceCollectors // gpu
	n.sharedState = c.State
}

func (n *nodeResourceCollector) Run(stopCh <-chan struct{}) {
	devicesSynced := func() bool {
		return over_framework.DeviceCollectorsStarted(n.deviceCollectors)
	}
...
func (n *nodeResourceCollector) collectNodeResUsed() {
	...
	memUsageMetrics, err := over_metriccache.NodeMemoryUsageMetric.GenerateSample(nil, collectTime, memUsageValue)
	if err != nil {
	...
	n.lastNodeCPUStat = &over_framework.CPUStat{
	...
	cpuUsageMetrics, err := over_metriccache.NodeCPUUsageMetric.GenerateSample(nil, collectTime, cpuUsageValue)
	if err != nil {
		klog.Warningf("generate node cpu metrics failed, err %v", err)
		return
	}
	...
	n.sharedState.UpdateNodeUsage(over_metriccache.Point{Timestamp: collectTime, Value: cpuUsageValue},
		over_metriccache.Point{Timestamp: collectTime, Value: memUsageValue})

	// update collect time
	n.started.Store(true)
	over_metrics.RecordNodeUsedCPU(cpuUsageValue) // in cpu cores

	klog.V(4).Infof("collectNodeResUsed finished, count %v, cpu[%v], mem[%v]", len(nodeMetrics), cpuUsageValue, memUsageValue)
}
