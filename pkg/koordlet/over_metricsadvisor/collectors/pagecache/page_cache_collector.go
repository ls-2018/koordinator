...
type pageCacheCollector struct {
	...
	metricDB              over_metriccache.MetricCache
...
func New(opt *over_framework.Options) over_framework.Collector {
	podFilter := opt.PodFilters[CollectorName]
...
func (p *pageCacheCollector) Setup(c *over_framework.Context) {}
...
func (p *pageCacheCollector) FilterPod(meta *over_statesinformer.PodMeta) (bool, string) {
...
func (p *pageCacheCollector) collectNodePageCache() {
	...
	nodeMetrics := make([]over_metriccache.MetricSample, 0)
	collectTime := timeNow()
	...
	memUsageWithPageCacheMetric, err := over_metriccache.NodeMemoryUsageWithPageCacheMetric.GenerateSample(nil, collectTime, memUsageWithPageCacheValue)
	if err != nil {
...
func (p *pageCacheCollector) collectPodPageCache() {
	...
	for _, meta := range podMetas {
		...
		memUsageWithPageCacheMetric, err := over_metriccache.PodMemoryUsageWithPageCacheMetric.GenerateSample(
			over_metriccache.MetricPropertiesFunc.Pod(uid), collectTime, float64(memUsageWithPageCacheValue))
		if err != nil {
...
func (p *pageCacheCollector) collectContainerPageCache(meta *over_statesinformer.PodMeta) []over_metriccache.MetricSample {
	...
	for i := range pod.Status.ContainerStatuses {
		...
		memUsageWithPageCacheMetric, err := over_metriccache.ContainerMemoryUsageWithPageCacheMetric.GenerateSample(
			over_metriccache.MetricPropertiesFunc.Container(containerStat.ContainerID), collectTime, float64(memUsagWithPageCacheValue))
		if err != nil {
