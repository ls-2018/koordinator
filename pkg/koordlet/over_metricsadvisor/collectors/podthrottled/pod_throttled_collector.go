...
func New(opt *over_framework.Options) over_framework.Collector {
	...
	return &podThrottledCollector{
		...
		podFilter:                 podFilter,
		lastPodCPUThrottled:       gocache.New(collectInterval*over_framework.ContextExpiredRatio, over_framework.CleanupInterval),
		lastContainerCPUThrottled: gocache.New(collectInterval*over_framework.ContextExpiredRatio, over_framework.CleanupInterval),
	}
}

var _ over_framework.PodCollector = &podThrottledCollector{}

func (c *podThrottledCollector) Enabled() bool {
	return true
}

func (c *podThrottledCollector) Setup(ctx *over_framework.Context) {}
...
func (c *podThrottledCollector) FilterPod(meta *over_statesinformer.PodMeta) (bool, string) {
...
func (c *podThrottledCollector) collectPodThrottledInfo() {
	...
	podAndContainerMetrics := make([]over_metriccache.MetricSample, 0)
	for _, meta := range podMetas {
		...
		podMetric, err := over_metriccache.PodCPUThrottledMetric.GenerateSample(
			over_metriccache.MetricPropertiesFunc.Pod(string(meta.Pod.UID)),
...
func (c *podThrottledCollector) collectContainerThrottledInfo(podMeta *over_statesinformer.PodMeta) []over_metriccache.MetricSample {
	pod := podMeta.Pod
	containersMetric := make([]over_metriccache.MetricSample, 0, len(pod.Status.ContainerStatuses))
	for i := range pod.Status.ContainerStatuses {
		...
		containerMetric, err := over_metriccache.ContainerCPUThrottledMetric.GenerateSample(
			over_metriccache.MetricPropertiesFunc.Container(containerStat.ContainerID),
			collectTime, cpuThrottledRatio)
		if err != nil {
