...
type beResourceCollector struct {
	...
	lastBECPUStat *over_framework.CPUStat
}
...
func (b *beResourceCollector) Setup(s *over_framework.Context) {
	return
}
...
func (b *beResourceCollector) collectBECPUResourceMetric() {
	...
	beLimit, err01 := over_metriccache.NodeBEMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.NodeBE(string(over_metriccache.BEResourceCPU), string(over_metriccache.BEResourceAllocationRealLimit)), collectTime, float64(realMilliLimit))
	beRequest, err02 := over_metriccache.NodeBEMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.NodeBE(string(over_metriccache.BEResourceCPU), string(over_metriccache.BEResourceAllocationRequest)), collectTime, float64(beCPUMilliRequest))
	beUsage, err03 := over_metriccache.NodeBEMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.NodeBE(string(over_metriccache.BEResourceCPU), string(over_metriccache.BEResourceAllocationUsage)), collectTime, float64(beCPUUsageMilliCores))
	...
	beMetrics := make([]over_metriccache.MetricSample, 0)
...
func (b *beResourceCollector) getBECPUUsageMilliCores() (int64, error) {
	...
	b.lastBECPUStat = &over_framework.CPUStat{
