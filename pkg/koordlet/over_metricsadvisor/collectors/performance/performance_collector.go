...
func (p *performanceCollector) Setup(s *over_framework.Context) {}
...
func (p *performanceCollector) collectContainerCPI() {
	...
	timeWindow := time.Now()
	containerStatusesMap := map[*corev1.ContainerStatus]*over_statesinformer.PodMeta{}
	...
	over_metrics.ResetContainerCPI()

	var wg1 sync.WaitGroup
	var mutex sync.Mutex
	wg1.Add(len(containerStatusesMap))
	cpiMetrics := make([]over_metriccache.MetricSample, 0)
...
func (p *performanceCollector) profileCPIOnSingleContainer(status *corev1.ContainerStatus, collectorOnSingleContainer perf.Collector, pod *corev1.Pod) []over_metriccache.MetricSample {
	...
	cpiCycle, err01 := over_metriccache.ContainerCPI.GenerateSample(over_metriccache.MetricPropertiesFunc.ContainerCPI(string(pod.GetUID()), status.ContainerID, string(over_metriccache.CPIResourceCycle)), collectTime, cycles)
	cpiInstruction, err02 := over_metriccache.ContainerCPI.GenerateSample(over_metriccache.MetricPropertiesFunc.ContainerCPI(string(pod.GetUID()), status.ContainerID, string(over_metriccache.CPIResourceInstruction)), collectTime, instructions)
	...
	over_metrics.RecordContainerCPI(status, pod, cycles, instructions)
...
func (p *performanceCollector) collectContainerPSI() {
	...
	over_metrics.ResetContainerPSI()

	var wg sync.WaitGroup
	var mutex sync.Mutex
	wg.Add(len(containerStatusesMap))
	psiMetrics := make([]over_metriccache.MetricSample, 0)
	for containerStatus, podMeta := range containerStatusesMap {
		pod := podMeta.Pod
...
func (p *performanceCollector) collectSingleContainerPSI(podParentCgroupDir string, containerStatus *corev1.ContainerStatus, pod *corev1.Pod) []over_metriccache.MetricSample {
	psiMetrics := make([]over_metriccache.MetricSample, 0)
	collectTime := time.Now()
	...
	cpuSomeAvg10, err01 := over_metriccache.ContainerPSIMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.ContainerPSI(string(pod.GetUID()), containerStatus.ContainerID, string(over_metriccache.PSIResourceCPU), string(over_metriccache.PSIPrecision10), string(over_metriccache.PSIDegreeSome)), collectTime, containerPSI.CPU.Some.Avg10)
	memSomeAvg10, err02 := over_metriccache.ContainerPSIMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.ContainerPSI(string(pod.GetUID()), containerStatus.ContainerID, string(over_metriccache.PSIResourceMem), string(over_metriccache.PSIPrecision10), string(over_metriccache.PSIDegreeSome)), collectTime, containerPSI.Mem.Some.Avg10)
	ioSomeAvg10, err03 := over_metriccache.ContainerPSIMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.ContainerPSI(string(pod.GetUID()), containerStatus.ContainerID, string(over_metriccache.PSIResourceIO), string(over_metriccache.PSIPrecision10), string(over_metriccache.PSIDegreeSome)), collectTime, containerPSI.IO.Some.Avg10)

	cpuFullAvg10, err04 := over_metriccache.ContainerPSIMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.ContainerPSI(string(pod.GetUID()), containerStatus.ContainerID, string(over_metriccache.PSIResourceCPU), string(over_metriccache.PSIPrecision10), string(over_metriccache.PSIDegreeFull)), collectTime, containerPSI.CPU.Full.Avg10)
	memFullAvg10, err05 := over_metriccache.ContainerPSIMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.ContainerPSI(string(pod.GetUID()), containerStatus.ContainerID, string(over_metriccache.PSIResourceMem), string(over_metriccache.PSIPrecision10), string(over_metriccache.PSIDegreeFull)), collectTime, containerPSI.Mem.Full.Avg10)
	ioFullAvg10, err06 := over_metriccache.ContainerPSIMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.ContainerPSI(string(pod.GetUID()), containerStatus.ContainerID, string(over_metriccache.PSIResourceIO), string(over_metriccache.PSIPrecision10), string(over_metriccache.PSIDegreeFull)), collectTime, containerPSI.IO.Full.Avg10)

	cpuFullSupported, err07 := over_metriccache.ContainerPSICPUFullSupportedMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.PSICPUFullSupported(string(pod.GetUID()), containerStatus.ContainerID), collectTime, tools.BoolToFloat64(containerPSI.CPU.FullSupported))
	...
	over_metrics.RecordContainerPSI(containerStatus, pod, containerPSI)
...
func (p *performanceCollector) collectSinglePodPSI(pod *corev1.Pod, podCgroupDir string) []over_metriccache.MetricSample {
	psiMetrics := make([]over_metriccache.MetricSample, 0)
	...
	cpuSomeAvg10, err01 := over_metriccache.PodPSIMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.PodPSI(string(pod.GetUID()), string(over_metriccache.PSIResourceCPU), string(over_metriccache.PSIPrecision10), string(over_metriccache.PSIDegreeSome)), collectTime, podPSI.CPU.Some.Avg10)
	memSomeAvg10, err02 := over_metriccache.PodPSIMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.PodPSI(string(pod.GetUID()), string(over_metriccache.PSIResourceMem), string(over_metriccache.PSIPrecision10), string(over_metriccache.PSIDegreeSome)), collectTime, podPSI.Mem.Some.Avg10)
	ioSomeAvg10, err03 := over_metriccache.PodPSIMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.PodPSI(string(pod.GetUID()), string(over_metriccache.PSIResourceIO), string(over_metriccache.PSIPrecision10), string(over_metriccache.PSIDegreeSome)), collectTime, podPSI.IO.Some.Avg10)

	cpuFullAvg10, err04 := over_metriccache.PodPSIMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.PodPSI(string(pod.GetUID()), string(over_metriccache.PSIResourceCPU), string(over_metriccache.PSIPrecision10), string(over_metriccache.PSIDegreeFull)), collectTime, podPSI.CPU.Full.Avg10)
	memFullAvg10, err05 := over_metriccache.PodPSIMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.PodPSI(string(pod.GetUID()), string(over_metriccache.PSIResourceMem), string(over_metriccache.PSIPrecision10), string(over_metriccache.PSIDegreeFull)), collectTime, podPSI.Mem.Full.Avg10)
	ioFullAvg10, err06 := over_metriccache.PodPSIMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.PodPSI(string(pod.GetUID()), string(over_metriccache.PSIResourceIO), string(over_metriccache.PSIPrecision10), string(over_metriccache.PSIDegreeFull)), collectTime, podPSI.IO.Full.Avg10)
	cpuFullSupported, err07 := over_metriccache.PodPSICPUFullSupportedMetric.GenerateSample(
		over_metriccache.MetricPropertiesFunc.Pod(string(pod.GetUID())), collectTime, tools.BoolToFloat64(podPSI.CPU.FullSupported))
	...
	over_metrics.RecordPodPSI(pod, podPSI)
...
func (p *performanceCollector) saveMetric(samples []over_metriccache.MetricSample) error {
	if len(samples) == 0 {
		return nil
	}
