...
func NewSharedState() *SharedState {
	return &SharedState{
		LatestMetric: LatestMetric{
			podsCPUByCollector:    make(map[string]over_metriccache.Point),
			podsMemoryByCollector: make(map[string]over_metriccache.Point),
		},
	}
}
...
type LatestMetric struct {
	...
	nodeCPU    *over_metriccache.Point
	nodeMemory *over_metriccache.Point

	podMutex              sync.RWMutex
	podsCPUByCollector    map[string]over_metriccache.Point
	podsMemoryByCollector map[string]over_metriccache.Point

	hostAppMutex  sync.RWMutex
	hostAppCPU    *over_metriccache.Point
	hostAppMemory *over_metriccache.Point
}

func (r *SharedState) UpdateNodeUsage(cpu, memory over_metriccache.Point) {
	r.nodeMutex.Lock()
...
func (r *SharedState) UpdatePodUsage(collectorName string, cpu, memory over_metriccache.Point) {
	r.podMutex.Lock()
...
func (r *SharedState) UpdateHostAppUsage(cpu, memory over_metriccache.Point) {
	r.hostAppMutex.Lock()
...
func (r *SharedState) GetNodeUsage() (cpu, memory *over_metriccache.Point) {
	r.nodeMutex.RLock()
	defer r.nodeMutex.RUnlock()
	return r.nodeCPU, r.nodeMemory
}

func (r *SharedState) GetHostAppUsage() (cpu, memory *over_metriccache.Point) {
	r.hostAppMutex.RLock()
	defer r.hostAppMutex.RUnlock()
	return r.hostAppCPU, r.hostAppMemory
}

func (r *SharedState) GetPodsUsageByCollector() (cpu, memory map[string]over_metriccache.Point) {
	r.podMutex.RLock()
	defer r.podMutex.RUnlock()
	podsCPU := map[string]over_metriccache.Point{}
	podsMemory := map[string]over_metriccache.Point{}
