...
type Rule struct {
	name            string
	description   string
	parseRuleType over_statesinformer.RegisterType
...
type UpdateCbFn func(target *over_statesinformer.CallbackTarget) error
...
func (r *Rule) runUpdateCallbacks(target *over_statesinformer.CallbackTarget) {
...
func UpdateRules(ruleType over_statesinformer.RegisterType, ruleObj interface{}, targets *over_statesinformer.CallbackTarget) {
	klog.V(4).Infof("applying %v rules with new %v, detail: %v", len(globalHookRules), ruleType.String(), util.DumpJSON(ruleObj))
