...
func (c *ContainerRequest) FromReconciler(podMeta *over_statesinformer.PodMeta, containerName string, sandbox bool) {
...
func (c *ContainerContext) FromReconciler(podMeta *over_statesinformer.PodMeta, containerName string, sandbox bool) {
...
func (c *ContainerContext) injectForOrigin() {
	...
	if c.Response.Resources.CPUShares != nil {
		eventHelper := over_audit.V(3).Container(c.Request.ContainerMeta.ID).Reason("runtime-hooks").Message(
	...
	if c.Response.Resources.CPUSet != nil && *c.Response.Resources.CPUSet != "" {
		eventHelper := over_audit.V(3).Container(c.Request.ContainerMeta.ID).Reason("runtime-hooks").Message("set container cpuset to %v", *c.Response.Resources.CPUSet)
