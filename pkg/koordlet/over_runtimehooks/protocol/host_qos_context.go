...
func (c *HostAppContext) injectForOrigin() {
	...
	if c.Response.Resources.CPUSet != nil && *c.Response.Resources.CPUSet != "" {
		eventHelper := over_audit.V(3).Group(c.Request.Name).Reason("runtime-hooks").Message("set host application cpuset to %v", *c.Response.Resources.CPUSet)
		updater, err := injectCPUSet(c.Request.CgroupParent, *c.Response.Resources.CPUSet, eventHelper, c.executor)
		if err != nil {
			klog.Infof("set host application %v cpuset %v on cgroup parent %v failed, error %v", c.Request.Name, *c.Response.Resources.CPUSet, c.Request.CgroupParent, err)
		} else {
			c.updaters = append(c.updaters, updater)
			klog.V(5).Infof("set host application %v cpuset %v on cgroup parent %v", c.Request.Name, *c.Response.Resources.CPUSet, c.Request.CgroupParent)
		}
	}
}
...
func (c *HostAppContext) injectForExt() {
	if c.Response.Resources.CPUBvt != nil {
		eventHelper := over_audit.V(3).Group(c.Request.Name).Reason("runtime-hooks").Message("set host application bvt to %v", *c.Response.Resources.CPUBvt)
		updater, err := injectCPUBvt(c.Request.CgroupParent, *c.Response.Resources.CPUBvt, eventHelper, c.executor)
		if err != nil {
			klog.Infof("set host application %v bvt %v on cgroup parent %v failed, error %v", c.Request.Name, *c.Response.Resources.CPUBvt, c.Request.CgroupParent, err)
		} else {
			c.updaters = append(c.updaters, updater)
			klog.V(5).Infof("set host application %v bvt %v on cgroup parent %v", c.Request.Name, *c.Response.Resources.CPUBvt, c.Request.CgroupParent)
		}
	}
}
