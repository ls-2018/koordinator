...
func (p *PodRequest) FromReconciler(podMeta *over_statesinformer.PodMeta) {
...
func (p *PodContext) FromReconciler(podMeta *over_statesinformer.PodMeta) {
...
func (p *PodContext) injectForExt() {
	if p.Response.Resources.CPUBvt != nil {
		eventHelper := over_audit.V(3).Pod(p.Request.PodMeta.Namespace, p.Request.PodMeta.Name).Reason("runtime-hooks").Message(
