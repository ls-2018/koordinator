...
import (
	...
	"github.com/koordinator-sh/koordinator/pkg/koordlet/over_runtimehooks/nri"
	"github.com/koordinator-sh/koordinator/pkg/koordlet/over_runtimehooks/proxyserver"
	...
	"github.com/koordinator-sh/koordinator/pkg/over_runtimeproxy/config"
)
...
func NewRuntimeHook(si over_statesinformer.StatesInformer, cfg *Config) (RuntimeHook, error) {
	...
	si.RegisterCallbacks(over_statesinformer.RegisterTypeNodeSLOSpec, "runtime-hooks-rule-node-slo", "Update hooks rule can run callbacks if NodeSLO spec update", rule.UpdateRules)
	si.RegisterCallbacks(over_statesinformer.RegisterTypeNodeTopology, "runtime-hooks-rule-node-topo", "Update hooks rule if NodeTopology info update", rule.UpdateRules)
	si.RegisterCallbacks(over_statesinformer.RegisterTypeNodeMetadata, "runtime-hooks-rule-node-metadata", "Update hooks rule if Node metadata update", rule.UpdateRules)
	si.RegisterCallbacks(over_statesinformer.RegisterTypeAllPods, "runtime-hooks-rule-all-pods", "Update hooks rule of all Pods refresh", rule.UpdateRules)
