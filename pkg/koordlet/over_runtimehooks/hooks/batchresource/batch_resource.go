...
import (
	...
	"github.com/koordinator-sh/koordinator/pkg/koordlet/over_runtimehooks/hooks"
	"github.com/koordinator-sh/koordinator/pkg/koordlet/over_runtimehooks/protocol"
	"github.com/koordinator-sh/koordinator/pkg/koordlet/over_runtimehooks/reconciler"
	"github.com/koordinator-sh/koordinator/pkg/koordlet/over_runtimehooks/rule"
	"github.com/koordinator-sh/koordinator/pkg/koordlet/over_statesinformer"
	"github.com/koordinator-sh/koordinator/pkg/koordlet/resourceexecutor"
	rmconfig "github.com/koordinator-sh/koordinator/pkg/over_runtimeproxy/config"
...
func (p *plugin) Register(op hooks.Options) {
	...
	rule.Register(ruleNameForNodeSLO, description, rule.WithParseFunc(over_statesinformer.RegisterTypeNodeSLOSpec, p.parseRuleForNodeSLO), rule.WithUpdateCallback(p.ruleUpdateCbForNodeSLO))
	rule.Register(ruleNameForNodeMeta, description, rule.WithParseFunc(over_statesinformer.RegisterTypeNodeMetadata, p.parseRuleForNodeMeta), rule.WithUpdateCallback(p.ruleUpdateCbForNodeMeta))
	...
	reconciler.RegisterCgroupReconciler(reconciler.PodLevel, system.CPUShares, description+" (pod cpu shares)", p.SetPodCPUShares, reconciler.PodQOSFilter(), podQOSConditions...)                         // ✅
	reconciler.RegisterCgroupReconciler(reconciler.PodLevel, system.CPUCFSQuota, description+" (pod cfs quota)", p.SetPodCFSQuota, reconciler.PodQOSFilter(), podQOSConditions...)                         // ✅
	reconciler.RegisterCgroupReconciler(reconciler.PodLevel, system.MemoryLimit, description+" (pod memory limit)", p.SetPodMemoryLimit, reconciler.PodQOSFilter(), podQOSConditions...)                   // ✅
	reconciler.RegisterCgroupReconciler(reconciler.ContainerLevel, system.CPUShares, description+" (container cpu shares)", p.SetContainerCPUShares, reconciler.PodQOSFilter(), podQOSConditions...)       // ✅
	reconciler.RegisterCgroupReconciler(reconciler.ContainerLevel, system.CPUCFSQuota, description+" (container cfs quota)", p.SetContainerCFSQuota, reconciler.PodQOSFilter(), podQOSConditions...)       // ✅
	reconciler.RegisterCgroupReconciler(reconciler.ContainerLevel, system.MemoryLimit, description+" (container memory limit)", p.SetContainerMemoryLimit, reconciler.PodQOSFilter(), podQOSConditions...) // ✅
...
func (p *plugin) SetPodCPUShares(proto protocol.HooksProtocol) error {
	...
	cpuShares := system.MilliCPUToShares(milliCPURequest)
...
func (p *plugin) SetPodCFSQuota(proto protocol.HooksProtocol) error {
	...
	cfsQuota := system.MilliCPUToQuota(milliCPULimit)
