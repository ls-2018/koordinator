...
func (b *bvtPlugin) Register(op hooks.Options) {
	...
	rule.Register(name, description, rule.WithParseFunc(over_statesinformer.RegisterTypeNodeSLOSpec, b.parseRule), rule.WithUpdateCallback(b.ruleUpdateCb), rule.WithSystemSupported(b.SystemSupported))
	reconciler.RegisterCgroupReconciler(reconciler.PodLevel, sysutil.CPUBVTWarpNs, "reconcile pod level cpu bvt value", b.SetPodBvtValue, reconciler.NoneFilter())             // ✅
	reconciler.RegisterCgroupReconciler(reconciler.KubeQOSLevel, sysutil.CPUBVTWarpNs, "reconcile kubeqos level cpu bvt value", b.SetKubeQOSBvtValue, reconciler.NoneFilter()) // ✅
	reconciler.RegisterHostAppReconciler(sysutil.CPUBVTWarpNs, "reconcile host application cpu bvt value", b.SetHostAppBvtValue, &reconciler.ReconcilerOption{})               // ✅
...
func (b *bvtPlugin) SystemSupported() bool {
	if b.sysSupported == nil {
		...
		klog.Infof("update system supported info to %v for plugin %v, supported msg %s", *b.sysSupported, name, msg)
	}
