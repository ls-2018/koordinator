...
func (p *Plugin) Register(op hooks.Options) {
	...
	rule.Register(name, description, rule.WithParseFunc(over_statesinformer.RegisterTypeNodeMetadata, p.parseRule), rule.WithUpdateCallback(p.ruleUpdateCb))
	...
	reconciler.RegisterCgroupReconciler(reconciler.PodLevel, sysutil.CPUCFSQuota, description+" (pod cfs quota)", p.AdjustPodCFSQuota, reconciler.PodQOSFilter(), podQOSConditions...)                   // ✅
	reconciler.RegisterCgroupReconciler(reconciler.ContainerLevel, sysutil.CPUCFSQuota, description+" (container cfs quota)", p.AdjustContainerCFSQuota, reconciler.PodQOSFilter(), podQOSConditions...) // ✅
