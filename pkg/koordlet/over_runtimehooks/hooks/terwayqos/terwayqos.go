...
func (p *Plugin) Register(op hooks.Options) {
	...
	rule.Register(ruleNameForNodeQoS, description, rule.WithParseFunc(over_statesinformer.RegisterTypeNodeSLOSpec, p.parseRuleForNodeSLO), rule.WithUpdateCallback(p.update))
	rule.Register(ruleNameForAllPods, description, rule.WithParseFunc(over_statesinformer.RegisterTypeAllPods, p.parseForAllPods), rule.WithUpdateCallback(p.update))
...
func (p *Plugin) update(target *over_statesinformer.CallbackTarget) error {
	if target == nil {
...
func (p *Plugin) syncPodConfig() error {
	...
	pEvent := over_audit.V(3).Node().Reason("netqos reconcile").Message("update pod to : %v", string(outPods))
...
func (p *Plugin) syncNodeConfig() error {
	...
	nEvent := over_audit.V(3).Node().Reason("netqos reconcile").Message("update node to : %v", string(outNode))
