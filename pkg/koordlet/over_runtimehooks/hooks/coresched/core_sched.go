...
func (p *Plugin) Register(op hooks.Options) {
	...
	rule.Register(ruleNameForNodeSLO, description, rule.WithParseFunc(over_statesinformer.RegisterTypeNodeSLOSpec, p.parseRuleForNodeSLO), rule.WithUpdateCallback(p.ruleUpdateCb), rule.WithSystemSupported(p.SystemSupported))
	rule.Register(ruleNameForAllPods, description, rule.WithParseFunc(over_statesinformer.RegisterTypeAllPods, p.parseForAllPods), rule.WithUpdateCallback(p.ruleUpdateCb), rule.WithSystemSupported(p.SystemSupported))
	reconciler.RegisterCgroupReconciler(reconciler.ContainerLevel, sysutil.VirtualCoreSchedCookie, "set core sched cookie to process groups of container specified", p.SetContainerCookie, reconciler.PodQOSFilter(), podQOSConditions...)       // ✅
	reconciler.RegisterCgroupReconciler(reconciler.SandboxLevel, sysutil.VirtualCoreSchedCookie, "set core sched cookie to process groups of sandbox container specified", p.SetContainerCookie, reconciler.PodQOSFilter(), podQOSConditions...) // ✅
	// TODO: support host application
	reconciler.RegisterCgroupReconciler(reconciler.KubeQOSLevel, sysutil.CPUIdle, "reconcile QoS level cpu idle", p.SetKubeQOSCPUIdle, reconciler.NoneFilter()) // ✅
	p.Setup(op)
}
...
func (p *Plugin) SystemSupported() bool {
	if p.sysSupported == nil {
		...
		p.supportedMsg = msg
		klog.Infof("update system supported info to %v for plugin %v, supported msg %s", sysSupported, name, msg)
	}
...
func (p *Plugin) initCache(podMetas []*over_statesinformer.PodMeta) bool {
...
func (p *Plugin) loadAllCookies(podMetas []*over_statesinformer.PodMeta) bool {
	hasSynced := false
	...
	for _, podMeta := range podMetas {
		...
		for _, cPID := range containerPIDs {
			...
			over_metrics.RecordContainerCoreSchedCookie(pod.Namespace, pod.Name, podUID, cPID.ContainerName, containerID,
				groupID, cookieID)
		}
	}
...
func (p *Plugin) enableContainerCookie(containerCtx *protocol.ContainerContext, groupID string) error {
	...
	if cookieEntry != nil {
		...
		over_metrics.RecordCoreSchedCookieManageStatus(groupID, false)
