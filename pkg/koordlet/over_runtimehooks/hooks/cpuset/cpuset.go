...
func (p *cpusetPlugin) Register(op hooks.Options) {
	...
	rule.Register(name, description, rule.WithParseFunc(over_statesinformer.RegisterTypeNodeTopology, p.parseRule), rule.WithUpdateCallback(p.ruleUpdateCb))

	reconciler.RegisterCgroupReconciler(reconciler.ContainerLevel, sysutil.CPUSet, "set container cpuset and unset container cpu quota if needed for cpuset pod", p.SetContainerCPUSetAndUnsetCFS, reconciler.PodQOSFilter(), cpusetPodQOSConditions...)       // ✅
	reconciler.RegisterCgroupReconciler(reconciler.SandboxLevel, sysutil.CPUSet, "set sandbox container cpuset and unset container cpu quota if needed for cpuset pod", p.SetContainerCPUSetAndUnsetCFS, reconciler.PodQOSFilter(), cpusetPodQOSConditions...) // ✅
	reconciler.RegisterCgroupReconciler(reconciler.PodLevel, sysutil.CPUCFSQuota, "unset pod cpu quota if needed for cpuset pod", UnsetPodCPUQuota, reconciler.PodQOSFilter(), cpusetPodQOSConditions...)                                                      // ✅
	reconciler.RegisterCgroupReconciler(reconciler.ContainerLevel, sysutil.CPUSet, "set container cpuset for cpushare pod is specified", p.SetContainerCPUSet, reconciler.PodQOSFilter(), cpusharePodQOSConditions...)                                         // ✅
	reconciler.RegisterCgroupReconciler(reconciler.SandboxLevel, sysutil.CPUSet, "set sandbox container cpuset for cpushare pod is specified", p.SetContainerCPUSet, reconciler.PodQOSFilter(), cpusharePodQOSConditions...)                                   // ✅

	reconciler.RegisterHostAppReconciler(sysutil.CPUSet, "set host application cpuset", p.SetHostAppCPUSet, &reconciler.ReconcilerOption{}) // ✅
