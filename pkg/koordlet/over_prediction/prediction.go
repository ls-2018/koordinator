...
func NewInformer(statesInformer over_statesinformer.StatesInformer) Informer {
	return &informer{
...
func NewMetricServer(metricCache over_metriccache.MetricCache, dataInterval time.Duration) MetricServer {
	return &metricServer{
...
func (ms *metricServer) GetPodMetric(desc MetricDesc, m MetricKey) (float64, error) {
	...
	podProperties := over_metriccache.MetricPropertiesFunc.Pod(string(desc.UID))
	queryPodMetric := func(m over_metriccache.MetricResource) (float64, error) {
		meta, err := m.BuildQueryMeta(podProperties)
		if err != nil {
			return 0, err
		}
		result := over_metriccache.DefaultAggregateResultFactory.New(meta)
		if err = querier.Query(meta, nil, result); err != nil {
			return 0, err
		}
		return result.Value(over_metriccache.AggregationTypeP90)
	}

	switch m {
	case CPUUsage:
		return queryPodMetric(over_metriccache.PodCPUUsageMetric)
	case MemoryUsage:
		return queryPodMetric(over_metriccache.PodMemUsageMetric)
	}
...
func (ms *metricServer) GetNodeMetric(desc MetricDesc, m MetricKey) (float64, error) {
	...
	queryNodeMetric := func(m over_metriccache.MetricResource) (float64, error) {
	...
	switch m {
	case CPUUsage:
		return queryNodeMetric(over_metriccache.NodeCPUUsageMetric)
	case MemoryUsage:
		return queryNodeMetric(over_metriccache.NodeMemoryUsageMetric)
	}
