...
type PredictServer interface {
	Setup(over_statesinformer.StatesInformer, over_metriccache.MetricCache) error
...
func (p *peakPredictServer) Setup(statesInformer over_statesinformer.StatesInformer, metricCache over_metriccache.MetricCache) error {
...
func (p *peakPredictServer) training() {
	...
	for _, pod := range pods {
		...
		lastCPUUsage, err := p.metricServer.GetPodMetric(MetricDesc{UID: uid}, CPUUsage) // 90%
		if err != nil {
