...
type ProcessorInfo struct {
	CPUID int32 `json:"cpu"` // logic CPU/处理器ID
...
func getCPUModel() (string, error) { // Intel(R) Xeon(R) Gold 6130 CPU @ 2.10GHz
...
// getHyperThreadEnabled 返回cpu是否启用ht
// 注意:目前只支持intel cpu,否则总是返回false
func getHyperThreadEnabled() (bool, error) {
	hyperThreadEnabledPath := system.GetSysCPUSMTActivePath() // 超线程
...
func getCPUTurboEnabled() (bool, error) { //
	//TODO:在当前版本中,只收集intel cpu的turbo状态。目前还不支持其他供应商的接口。我们以后可以检查频率。
	turboDisabledPath := system.GetSysIntelPStateNoTurboPath() // /sys/devices/system/cpu/intel_pstate/no_turbo
...
func getCPUBasicInfo() (*extension.CPUBasicInfo, error) {
	...
	var err error
	if cpuBasicInfo.CPUModel, err = getCPUModel(); err != nil { // getCPUModel gets the Model name of the CPU.
		klog.V(4).Infof("get cpu model error: %v", err)
	}
	if cpuBasicInfo.HyperThreadEnabled, err = getHyperThreadEnabled(); err != nil { // 是否开启超线程
		klog.V(4).Infof("get hyperthreadEnabled info error: %v", err)
	}
	if cpuBasicInfo.TurboEnabled, err = getCPUTurboEnabled(); err != nil { // cpu 超频
		klog.V(4).Infof("get TurboEnabled info error: %v", err)
	}
	if cpuBasicInfo.CatL3CbmMask, err = system.ReadCatL3CbmString(); err != nil { // /sys/fs/resctrl/info/L3/cbm_mask
